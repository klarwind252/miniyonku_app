...
router = APIRouter()
# BASE_DIR は静的アセットのパス計算等で使う可能性があるため、app/ を指すよう再計算する
# （このファイルは app/presentation/routers/ にあるので3階層上がって app/ の親→appは dirname×2 で app/presentation、×3 で app/ の親。
#   旧実装は app/ を指していたため、それに合わせる）
...
async def viewer_qualifying(tid: int, request: Request, db: aiosqlite.Connection = Depends(get_db)):
    ...
    m4laps_rows = []
    m4laps_best_ranks = {}
    _m4_scan_vq = None  # 予選のみの一括スキャン結果（後段の RECORD HOLDERS でも再利用）
    if IS_CLOUD and await m4laps_license.is_licensed(db):
        from app.application import timing_racer_best_service as _rbest
        try:
            _m4_scan_vq = await _rbest.scan_tournament_metrics(db, tid)
            _rbests = _m4_scan_vq["bests"]
        except Exception:
            _rbests = {}
        if _rbests:
            # entry_id -> レーサー名
    ...
    # RECORD HOLDERS：上の M4LAPS ベスト表と同じ一括スキャン結果を再利用（再走査しない）
    _rh_raw = _m4_scan_vq["records"] if _m4_scan_vq else None
    ...
    _qual_leader_eid = _pl_eid
    if _qual_leader_eid is None and qt == "order" and standings:
        _olead = [s for s in standings if s.get("rank") == 1]
        if _olead:
            _obt = {s["entry_id"]: s.get("best_total")
                    for s in standings if s.get("entry_id") is not None}
            _ow = await _qrec.resolve_point_leader(db, tid, _olead, _obt)
...
async def viewer_bracket(tid: int, request: Request, db: aiosqlite.Connection = Depends(get_db)):
    ...
    #   GRAND SLAM 判定／RECORD HOLDERS パネル／⚡M4LAPS ベスト表がそれぞれ
    #   bests・records を必要とするため、遅延メモ化で共有する（従来は同じ
    #   全レース走査を最大5回繰り返していた）。
    _m4_scan_memo: dict = {}

    async def _get_m4_scan():
        if "v" not in _m4_scan_memo:
            from app.application import timing_racer_best_service as _rbsc
            _m4_scan_memo["v"] = await _rbsc.scan_tournament_metrics(
    ...
    if _champ_eid is not None:
        try:
            ...
            _scan_gs = await _get_m4_scan()
            _rh = _scan_gs["records"]
    ...
    if IS_CLOUD and await m4laps_license.is_licensed(db):
        try:
            ...
            _scan_rh = await _get_m4_scan()
            _rh2 = _scan_rh["records"]
            _nm2 = {}
            async with db.execute(
                "SELECT e.id AS entry_id, r.name FROM entries e "
                "JOIN racers r ON r.id=e.racer_id WHERE e.tournament_id=?",
    ...
    if IS_CLOUD and await m4laps_license.is_licensed(db):
        from app.application import timing_racer_best_service as _rbest
        try:
            _rbests = (await _get_m4_scan())["bests"]
        except Exception:
            _rbests = {}
        if _rbests:
            _name_by_eid = {}
            ...
            _fr_by = {}
            try:
                _fr_by = await _rbest.qualifying_finish_rates(db, tid)
            except Exception:
                _fr_by = {}
            # GAP 基準（TOTAL 最小）
            _best_total_min = None
